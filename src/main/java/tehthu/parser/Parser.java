...
public class Parser {
	...
	private Rope translate(Rope r, Direction d) {
		...
		for(Entry<String, Lang> elt : elements) {
			...
			if(l == Lang.L || l == Lang.R) {
				...
				if(tword == null) {
					//No match - don't know how to translate word, so add it back in untranslated
					retval.add(s, l);
				}
				else {
	...
	private Rope sanitizeAndRope(String input, Direction d) {
		...
		for(String s : tokens) {
			...
			if(m.matches()) { //If there are leading or trailing special characters
				String specialPre = m.group(2),
						specialSuff = m.group(6),
